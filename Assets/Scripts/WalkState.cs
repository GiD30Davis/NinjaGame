public override void Tick(float deltaTime)
{
    ...
    // --- NEW: Flip player direction based on input ---
    if (moveInput.x > 0)
    {
        stateMachine.transform.localScale = new Vector3(1, 1, 1); // Face right
    }
    else if (moveInput.x < 0)
    {
        stateMachine.transform.localScale = new Vector3(-1, 1, 1); // Face left
    }
...
public override void Tick(float deltaTime)
{
    ...
    if (stateMachine.InputReader.IsVPressed())
    {
